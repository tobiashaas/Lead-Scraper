...
login_response = requests.post(
    ...
    json={"username": "tobiashaas", "password": "HaasMal!a123"}
)
...
if login_response.status_code == 200:
    ...
    stats_response = requests.get(
        f"{API_URL}/api/v1/export/companies/stats",
        headers=headers
    )
    ...
    json_response = requests.get(
        f"{API_URL}/api/v1/export/companies/json?limit=5",
        headers=headers
    )
    ...
    csv_response = requests.get(
        f"{API_URL}/api/v1/export/companies/csv?limit=5",
        headers=headers
    )
