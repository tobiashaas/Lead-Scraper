...
async def main():
    ...
    enriched_results = await enrich_with_details(
        results=results, use_tor=use_tor, max_details=max_details
    )
    ...
    enriched_results = await find_missing_websites(
        results=enriched_results, use_tor=use_tor, max_searches=3  # Nur 3 für Demo
    )
    ...
    print(
        f"   - Websites gefunden: {sum(1 for r in enriched_results if r.website and 'google.com' not in r.website)} von {len(enriched_results)}"
    )
    print(
        f"   - Davon via Google Search: {sum(1 for r in enriched_results if r.website and r.extra_data.get('website_source') == 'google_search')}"
    )
    print()
