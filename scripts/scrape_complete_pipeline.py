...
async def main():
    ...
    results = await enrich_with_details(
        results=results,
        use_tor=use_tor,
    ...
    try:
        results = await enrich_with_google_places(
            results=results,
            max_lookups=max_google_places
        )
    ...
    results = await enrich_with_website_data(
        results=results,
        max_scrapes=max_website_scrapes
    )
    ...
    results = await enrich_with_handelsregister(
        results=results,
        use_tor=True,
        max_lookups=max_handelsregister,
