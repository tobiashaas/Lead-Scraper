...
class Deduplicator:
    ...
    def calculate_similarity(
        ...
        name_sim = fuzz.token_sort_ratio(
            company_a.company_name.lower(),
            company_b.company_name.lower()
        )
        ...
        if company_a.address and company_b.address:
            address_sim = fuzz.token_sort_ratio(
                company_a.address.lower(),
                company_b.address.lower()
            )
        ...
        if company_a.website and company_b.website:
            website_sim = fuzz.ratio(
                company_a.website.lower(),
                company_b.website.lower()
            )
    ...
    def is_duplicate(
        ...
        """
        name_sim, address_sim, phone_sim, website_sim, overall_sim = \
            self.calculate_similarity(company_a, company_b)
        ...
        if name_sim >= self.name_threshold:
            if (address_sim >= self.address_threshold or
                phone_sim >= self.phone_threshold or
                website_sim >= self.website_threshold or
    ...
    def merge_companies(
        ...
        merge_fields = [
            ...
            'legal_form', 'industry', 'directors', 'services', 'technologies'
        ]
        ...
        if duplicate.extra_data:
            ...
            if 'sources' in duplicate.extra_data:
                ...
                primary.extra_data['sources'].extend(
                    duplicate.extra_data['sources']
                )
