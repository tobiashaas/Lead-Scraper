...
class WebsiteScraper:
    ...
    async def scrape_website(self, url: str) -> Dict:
        ...
        data = {"impressum": None, "team": [], "emails": [], "phones": []}
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                for link in links:
                    ...
                    # Impressum
                    if not pages.get("impressum") and any(
                        word in text for word in ["impressum", "imprint", "legal"]
                    ):
                        pages["impressum"] = urljoin(base_url, href)

                    # Team
                    if not pages.get("team") and any(
                        word in text for word in ["team", "über uns", "about", "mitarbeiter"]
                    ):
                        pages["team"] = urljoin(base_url, href)

                    # Kontakt
                    if not pages.get("kontakt") and any(
                        word in text for word in ["kontakt", "contact", "ansprechpartner"]
                    ):
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                court_match = re.search(
                    r"(?:Amtsgericht|Registergericht)\s+([A-ZÄÖÜ][a-zäöüß]+)", text
                )
                if court_match:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                member_containers = soup.find_all(
                    ...
                    and any(
                        word in str(x).lower()
                        for word in ["team", "member", "person", "mitarbeiter"]
                    ),
                )
                ...
                for container in member_containers[:20]:  # Max 20
                    ...
                    role_tag = container.find(
                        text=re.compile(r"(Geschäftsführer|CEO|CTO|Manager|Leiter|Developer)", re.I)
                    )
                    if role_tag:
    ...
    for result in results:
        ...
        try:
            ...
            if website_data:
                ...
                if fields:
                    result.add_source("website_scraping", result.website, fields)
