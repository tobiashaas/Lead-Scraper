...
class GoYellowScraper(BaseScraper):
    ...
    def _parse_entry(self, entry, source_url: str) -> ScraperResult:
        ...
        if address_container:
            # Straße
            street_tag = address_container.find('span', class_=lambda x: x and 'street' in str(x).lower())
            if street_tag:
                street = street_tag.get_text(strip=True)
            
            # PLZ + Stadt
            city_tag = address_container.find('span', class_=lambda x: x and 'city' in str(x).lower())
            if city_tag:
