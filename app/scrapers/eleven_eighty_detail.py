...
class ElevenEightyDetailScraper:
    ...
    def _extract_contact_persons(self, soup: BeautifulSoup) -> Optional[List[Dict]]:
        ...
        for container in person_containers:
            ...
            # Name
            name_tag = container.find(['h3', 'h4', 'span'], class_=lambda x: x and 'name' in str(x).lower())
            if name_tag:
                person['name'] = name_tag.get_text(strip=True)
            
            # Position/Rolle
            role_tag = container.find('span', class_=lambda x: x and ('role' in str(x).lower() or 'position' in str(x).lower()))
            if role_tag:
