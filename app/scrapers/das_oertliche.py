...
class DasOertlicheScraper(BaseScraper):
    ...
    def __init__(self, use_tor: bool = True):
        super().__init__(
            name="das_oertliche", domain="dasoertliche.de", use_tor=use_tor, use_playwright=True
        )
    ...
    def _parse_entry(self, entry, source_url: str) -> ScraperResult:
        ...
        # PLZ + Stadt
        city_tag = entry.find(
            "span", class_=lambda x: x and ("city" in str(x).lower() or "ort" in str(x).lower())
        )
        if city_tag:
        ...
        # Website
        website = None
        website_tag = entry.find(
            "a",
            class_=lambda x: x and ("website" in str(x).lower() or "homepage" in str(x).lower()),
        )
