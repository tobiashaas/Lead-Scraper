...
class TestCompaniesEndpoints:
    ...
    def test_create_company_duplicate(self, sample_company_data, client, auth_headers):
        ...
        response1 = client.post("/api/v1/companies/", json=sample_company_data, headers=auth_headers)
        assert response1.status_code == 201
        
        # Try to create duplicate
        response2 = client.post("/api/v1/companies/", json=sample_company_data, headers=auth_headers)
    ...
    def test_update_company(self, create_test_company, client, auth_headers):
        ...
        response = client.put(f"/api/v1/companies/{company.id}", json=update_data, headers=auth_headers)
